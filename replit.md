...
**Module ↔ scenario integrity:** The training SPA (`app.html`) renders each module's practice scenarios by resolving `modules[].scenarioIds` against `practiceScenarios` in `public/js/content.js`; `lib/tracks.js` `MODULE_SCENARIOS` mirrors the same mapping for server-side gating — **keep the two in lockstep**. Verify with `node scripts/check-module-scenarios.js` (the `module-scenarios` validation) after any module rename or scenario id change; it also runs an advisory (never-failing) topic-drift pass. Most modules carry exactly **5** scenarios; modules 15/16 carry 6 and module 25 is a 9-scenario bar/cocktail deep-dive by design (don't "balance" these). Full detail — drift heuristics (island/cluster), the reviewed false-positive allowlist, and the deliberate count distribution — lives in `docs/module-scenario-integrity.md`.
