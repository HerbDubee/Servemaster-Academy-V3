...
**The signed-URL fix:** `lib/audioObjectStore.js#createAudioStore` exposes `signedUrl(key, ttlSec=3600)` → checks `isConfigured()` → `exists()` → mints a signed GET URL via the Replit sidecar (`POST /object-storage/signed-object-url`); returns `null` if unconfigured/missing (so the route falls through to local cache / synth), throws only on an unexpected sign failure. TTL = 6h covers long listens/seeking; one signed URL serves all subsequent Range requests, and bytes flow browser↔GCS directly with native Range/206. `storage.googleapis.com` is the signed-URL host.

**CRITICAL ordering — redirect BEFORE local `sendFile`:** `res.sendFile` of a large cached MP3 is **just as broken** on autoscale as streaming — the proxy 500s the large body the same way (HEAD/bounded-range succeed, open-ended `bytes=0-`/full GET 500). So in both `/api/books/tts/:key` and `/api/blog/tts/:lang/:slug` the route order is: (1) signed-URL `res.redirect(302)` when `isConfigured()` && object exists, (2) local-cache `sendFile` **fallback only** (storage unconfigured in dev, or object not yet uploaded), (3) live synth. Do NOT put the local `sendFile` fast-path first — that was the bug behind "Book 4 only first 3 chapters play": the deploy bundles the gitignored `books/audio-cache/*.mp3` working-tree files, so any chapter that happened to have a local file hit `sendFile`→proxy-500, while chapters with no local file fell through to the working redirect. **Lesson: on autoscale, NEVER serve large media through the app by any means (stream OR sendFile) — always redirect to a signed Object Storage URL.**

**Deploy quirk:** the autoscale deploy includes the **working-tree** copy of gitignored files (e.g. `books/audio-cache/*.mp3`), so whatever happens to be on disk at publish time ships to prod — which is exactly why some chapters had a local cache file in prod and others didn't.
