...
app.post('/api/auth/login', async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign({ id: user.id, email: user.email, name: user.name, role: user.role }, JWT_SECRET, { expiresIn: '30d' });
    res.cookie('token', token, { httpOnly: true, maxAge: 30 * 24 * 3600 * 1000, sameSite: 'lax' });
    res.json({ user: { id: user.id, name: user.name, email: user.email, role: user.role, experience_level: user.experience_level }, token });
  } catch (err) {
...
app.get('/api/auth/me', authMiddleware, async (req, res) => {
  try {
    const result = await db.query('SELECT id, name, email, role, experience_level, restaurant_id FROM users WHERE id = $1', [req.user.id]);
...
app.post('/api/manager/create-restaurant', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const result = await db.query(
      'INSERT INTO restaurants (name, owner_id, invite_code) VALUES ($1, $2, $3) RETURNING *',
      [restaurantName, req.user.id, inviteCode]
    );
...
app.post('/api/manager/join', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: 'Failed to join restaurant' });
  }
});
...
app.listen(PORT, '0.0.0.0', () => console.log(`ServeMaster Academy running on port ${PORT}`));
